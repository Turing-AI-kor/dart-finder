"""
DART Finder Web UI v2
- 분석은 백그라운드에서 실행 (WebSocket 끊겨도 계속)
- 상태를 파일에 저장 (새로고침/재접속해도 이어보기)
- 폴링 방식 (WebSocket 안 씀)
"""
import asyncio
import csv
    ...
    if STATE_FILE.exists():
        try:
            ...
            for k, v in data.items():
                job_state[k] = v
            # 서버 재시작 시 running 상태면 멈춘 것으로 처리
...
<style>
  ...
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  .header{padding:24px 32px;border-bottom:1px solid var(--border);display:flex;align-items:center;gap:16px;background:var(--surface)}
  ...
  .sidebar{background:var(--surface);border-right:1px solid var(--border);padding:24px;overflow-y:auto;display:flex;flex-direction:column;gap:24px}
  .section-title{font-family:'JetBrains Mono',monospace;font-size:11px;font-weight:600;color:var(--muted);letter-spacing:2px;text-transform:uppercase;margin-bottom:12px}
  .form-group{margin-bottom:16px}
  label{display:block;font-size:12px;color:var(--muted);margin-bottom:6px;font-weight:500}
  input,select{width:100%;background:var(--bg);border:1px solid var(--border);color:var(--text);padding:8px 12px;border-radius:6px;font-size:13px;font-family:'JetBrains Mono',monospace;transition:border-color 0.2s}
  input:focus,select:focus{outline:none;border-color:var(--accent)}
  .btn{width:100%;padding:12px;border:none;border-radius:8px;font-size:14px;font-weight:600;cursor:pointer;transition:all 0.2s;font-family:'Pretendard',sans-serif}
  ...
  .log-box{background:var(--bg);border:1px solid var(--border);border-radius:8px;padding:12px;height:160px;overflow-y:auto;font-family:'JetBrains Mono',monospace;font-size:11px;line-height:1.8}
  ...
  .toolbar{padding:16px 24px;border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between;gap:12px;background:var(--surface)}
  .stats{display:flex;gap:24px}
  .stat{text-align:center}
  .stat-val{font-family:'JetBrains Mono',monospace;font-size:20px;font-weight:700;color:var(--accent)}
  .stat-label{font-size:11px;color:var(--muted);margin-top:2px}
  .tier-badge{display:inline-block;padding:2px 8px;border-radius:4px;font-family:'JetBrains Mono',monospace;font-size:11px;font-weight:700}
  ...
  th{padding:10px 16px;text-align:left;font-family:'JetBrains Mono',monospace;font-size:11px;color:var(--muted);border-bottom:1px solid var(--border);background:var(--surface);position:sticky;top:0;white-space:nowrap;font-weight:600;letter-spacing:1px;text-transform:uppercase}
  td{padding:10px 16px;border-bottom:1px solid rgba(30,30,46,0.5);white-space:nowrap;max-width:200px;overflow:hidden;text-overflow:ellipsis}
  tr:hover td{background:rgba(255,255,255,0.02)}
  .score-bar{display:flex;align-items:center;gap:8px}
  .score-mini{height:4px;border-radius:2px;background:var(--accent);min-width:4px}
  .rank-num{font-family:'JetBrains Mono',monospace;font-size:12px;color:var(--muted);width:32px}
  ...
  .empty-text{font-family:'JetBrains Mono',monospace;font-size:13px}
  .dl-btn{padding:8px 16px;background:transparent;border:1px solid var(--accent);color:var(--accent);border-radius:6px;font-size:13px;font-weight:600;cursor:pointer;text-decoration:none;display:inline-flex;align-items:center;gap:6px}
  .dl-btn:hover{background:rgba(0,255,136,0.1)}
  .dl-btn.hidden{display:none}
  .filter-tabs{display:flex;gap:4px}
  .tab{padding:6px 12px;border-radius:6px;font-size:12px;font-weight:600;cursor:pointer;border:1px solid var(--border);background:transparent;color:var(--muted);font-family:'JetBrains Mono',monospace}
...
<div class="main">
  <div class="sidebar">
    <div>
      ...
      <div class="form-group"><label>최소 매출 (억원)</label><input type="number" id="revenueMin" value="100" min="0"></div>
      <div class="form-group"><label>최대 매출 (억원)</label><input type="number" id="revenueMax" value="5000" min="0"></div>
      <div class="form-group"><label>최소 직원수</label><input type="number" id="empMin" value="30" min="1"></div>
      <div class="form-group"><label>최대 직원수</label><input type="number" id="empMax" value="999999" min="1"></div>
      <div class="form-group"><label>추출 수 (최대)</label><input type="number" id="top" value="5000" min="1"></div>
      <div class="form-group"><label>샘플 크기 (0=전체)</label><input type="number" id="sampleSize" value="15000" min="0"></div>
  ...
  <div class="content">
    <div class="toolbar">
      <div class="stats">
        <div class="stat"><div class="stat-val" id="statTotal">0</div><div class="stat-label">전체</div></div>
        <div class="stat"><div class="stat-val" style="color:var(--tier-s)" id="statS">0</div><div class="stat-label">Tier S</div></div>
        <div class="stat"><div class="stat-val" style="color:var(--tier-a)" id="statA">0</div><div class="stat-label">Tier A</div></div>
        <div class="stat"><div class="stat-val" style="color:var(--tier-b)" id="statB">0</div><div class="stat-label">Tier B</div></div>
      </div>
      <div style="display:flex;gap:8px;align-items:center">
        <div class="filter-tabs">
          <button class="tab active" onclick="filterTier('all',event)">전체</button>
          <button class="tab" onclick="filterTier('S',event)">S</button>
          <button class="tab" onclick="filterTier('A',event)">A</button>
          <button class="tab" onclick="filterTier('B',event)">B</button>
        </div>
    ...
    <div class="table-wrap" id="tableWrap">
      ...
      <table id="resultTable" style="display:none">
        <thead><tr>
          <th>#</th><th>Tier</th><th>회사명</th><th>점수</th><th>뚫림</th><th>매출력</th>
          <th>매출(억)</th><th>마진%</th><th>성장%</th><th>직원</th><th>설립</th>
          <th>추천컨택</th><th>영업채널</th><th>도메인</th><th>이메일후보</th><th>주소</th>
        </tr></thead>
...
async function pollStatus() {
  try {
    ...
    if (s.running) {
      ...
      document.getElementById('statusText').textContent = `분석 중 - ${s.phase}`;
      ...
      document.getElementById('statusText').textContent = s.phase === 'error' ? '에러' : (s.phase === 'stopped' ? '중단됨' : '서버 재시작으로 중단됨');
      document.getElementById('runBtn').disabled = false;
      document.getElementById('stopBtn').style.display = 'none';
    } else {
      document.getElementById('statusDot').className = 'status-dot';
    ...
    if (s.results_count !== lastResultsCount) {
...
function renderTable(results) {
  ...
  tbody.innerHTML = results.map(r => `
    <tr>
      ...
      <td><div class="score-bar"><div class="score-mini" style="width:${Math.max(4,r.score*1.8)}px"></div><span style="font-family:'JetBrains Mono',monospace;font-size:12px">${r.score}</span></div></td>
...
@app.get("/download")
async def download():
    files = list(Path("output").glob("targets_*.csv")) if Path("output").exists() else []
    if not files:
...
async def _async_analyze(params):
    ...
    sample_size = params.get("sample_size", 15000)
    ...
    async def _async_get(client, path, p):
        try:
            r = await client.get(path, params={"crtfc_key": settings.OPENDART_API_KEY, **p}, timeout=30.0)
            r.raise_for_status()
            return r.json()
        except Exception:
            return None
